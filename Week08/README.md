...
Q15) 스프링 컨테이너는 등록된 스프링 빈을 Singleton 패턴으로 생성하고 관리한다. 각 메서드에 @Bean을 붙여 스프링 컨테이너에 스프링 빈으로 등록할 수 있다.

Q16) 필터와 인터셉터는 Web Spring영역에 있다. (O/X) 필터는 Web Spring영역에 있지 않다.
